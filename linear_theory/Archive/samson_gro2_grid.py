...
def init_parameter_arrays():
    global M, ndensc, ndensw, temperp, A
    # Index 1 denotes hydrogen ; 2 denotes helium; 3 denotes oxygen etc. (Incorrect?)
    M    = np.zeros(N)
    M[0] = 1.0              # Hydrogen
    M[1] = 4.0              # Helium
    M[2] = 16.0             # Oxygen
...
def constants():
    global PMASS, MUNOT, EVJOULE, CHARGE
    PMASS   = 1.67E-27
    MUNOT   = 1.25660E-6
    EVJOULE = 6.242E18
    CHARGE  = 1.60E-19
...
if __name__ == '__main__':
    ...
    normalize_amplitude = False                       # Normalize amplitude? (0: No, 1: Yes)
    normalize_frequency = True
    maxfreq   = 0.5                                   # Multiple of cyclotron frequency to calculate up to. Must be <= 1.0
...
#######################################
    ...
    for i in range(N):                                              # Loop over 3 species of ions
        ndensc[i]  = ndensc[i]*1.0E6                                # Cast density to /m3
        ndensw[i]  = ndensw[i]*1.0E6                                # Cast density to /m3
...
## NORMALIZE IF NEEDED ## 
    ...
    for ii in range(NPTS):
        ...
        if growth[ii] > themax:
            themax = growth[ii]                      # What is "themax" ?

    if normalize_amplitude == True:
        for k in range(NPTS):

...
def get_mag_data(time_start, time_end, probe, low_cut=None, high_cut=None):
    ...
    #time_sec     = (ti - ti[0]) / np.timedelta64(1, 's')
    #fft_time_sec = (_xtime - ti[0]) / np.timedelta64(1, 's')
    #start_sec    = (time_start - ti[0]) / np.timedelta64(1, 's')
    #end_sec      = (time_end - ti[0]) / np.timedelta64(1, 's')
    return ti, dat, dt, _xtime, _xfreq, _pow
...
def load_EMIC_IMFs_and_dynspec(imf_start, imf_end, IA_filter=None):
    ...
    '''
    _ti, _dat, _dt, _xtime, _xfreq, _pow = get_mag_data(_time_start, _time_end, 
...
# =============================================================================
    ...
    for ii in range(3):
        ...
        IFs[ii] = IFs[ii][st:en]
        IPs[ii] = IPs[ii][st:en]
    return _ti, _dat, _imf_t, IAs, IFs, IPs, _xtime, _xfreq, _pow
...
def calculate_all_NL_amplitudes():
    ...
    mag_time, pc1_mags, imf_time, IA, IF, IP, stime, sfreq, spower = \
...
def plot_velocities_and_energies(time_start, time_end, probe='a'):
    # Import cutoff-derived composition information
    cutoff_filename = 'D://Google Drive//Uni//PhD 2017//Josh PhD Share Folder//Thesis//Data_Plots//20130725_RBSP-A//pearl_times.txt'
    cutoff_dict     = epd.read_cutoff_file(cutoff_filename)
...
if __name__ == '__main__':
    ...
    pc1_res    = 35.0
    if True:
        _time_start = np.datetime64('2013-07-25T21:26:00')
        _time_end   = np.datetime64('2013-07-25T21:45:00')
    ...
    plot_velocities_and_energies(_time_start, _time_end, probe=_probe)
...
# =============================================================================
    ...
    # =============================================================================
    #     axes[1].semilogy(f_vals, T_N, c='k', ls='-', label='$\\tau=${}'.format(tau))
    #     axes[1].set_ylabel('$T_N \Omega_{H0}$')
    #     axes[1].set_xlabel('$\omega/\Omega_{H0}$')
    #     axes[1].set_xlim(f_vals[0], f_vals[-1])

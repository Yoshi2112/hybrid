...
def get_dispersion_relation(field, ndensc, ndensw, A, t_perp, norm_k=False, norm_w=False, Nk=1000, kmin=0.0, kmax=1.0, k_input_norm=1, plot=False, save=False, savepath=None):
    '''
    ...
    norm_k -- Flag: Normalize wavenumber to units of vA/p_cyc
    ...
    NOTE: STILL HAVE TO TEST HOW IT GOES WITH A H+, O+ mix
    '''
    global c, k_vals, N
    ...
    CPDR_solns           = np.ones((Nk, N   )) * eps
    warm_solns           = np.ones((Nk, N, 2)) * eps
    ...
    if plot==True or save==True:
        plot_dispersion(k_vals, CPDR_solns, warm_solns, norm_k=norm_k, norm_w=norm_w, plot=plot,
                          save=save, savepath=savepath)
    
    return k_vals, CPDR_solns, warm_solns


def plot_dispersion(k_vals, CPDR_solns, warm_solns, norm_k=False, norm_w=False, plot=False, save=False, savepath=None):
    '''
    docstring
    '''
    ...
    type_label = ['Cold Plasma Approx.', 'Warm Plasma Approx.', 'Cyclotron Frequencies']
    type_style = ['--', '-', ':']
...
if __name__ == '__main__':
    ...
    kval, cpdr, warm_dr = get_dispersion_relation(tfield, tndensc, tndensw, tA, tt_perp, norm_k=False, norm_w=False, plot=True, kmax=3.0)    
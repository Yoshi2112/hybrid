...
def plot_growth_rate_with_time(times, k_vals, all_WPDR, save=False, short=False,
                               norm_w=False, B0=None, ccomp=[70, 20, 10], suff=''):
    tick_label_size = 14
    ...
    if short == True:
        ...
        figsave_path = save_dir + '_LT_timeseries_CC_{:03}_{:03}_{:03}_{}_{}_short.png'.format(ccomp[0], ccomp[1], ccomp[2], save_string, suff)
    else:
        ax1.set_xlim(time_start, time_end)
        figsave_path = save_dir + '_LT_timeseries_CC_{:03}_{:03}_{:03}_{}_{}.png'.format(ccomp[0], ccomp[1], ccomp[2], save_string, suff)
...
def load_and_plot_timeseries():
    ...
    times, B0, name, mass, charge, density, tper, ani, cold_dens = \
        extract_species_arrays(time_start, time_end, probe, pad, return_raw_ne=True)
    
    suffixes = ['_newtime_nofilt', '_newtime_filt', '_oldtime_nofilt', '_oldtime_filt']
    
    for suff in suffixes:
        this_file = 'DR_results_coldcomp_070_020_010_20130725_2100_2200{}.npz'.format(suff)
        ...
        for file in files:
            if file[-4:] == '.npz':
                ...
                plot_growth_rate_with_time(times, all_k, all_WPDR, save=True,
                                           short=True, norm_w=False, B0=B0, ccomp=ccomp,
                                           suff=suff)
    return
...
def plot_all_DRs(ccomp=[70, 20, 10]):
    ...
    files = ['DR_results_coldcomp_070_020_010_20130725_2100_2200_newtime_nofilt.npz',
             'DR_results_coldcomp_070_020_010_20130725_2100_2200_newtime_filt.npz',
             'DR_results_coldcomp_070_020_010_20130725_2100_2200_oldtime_nofilt.npz',
             'DR_results_coldcomp_070_020_010_20130725_2100_2200_oldtime_filt.npz'] 
    ...
    for file in files:
        ...
        for ii in range(times.shape[0]):
            ...
            PlasParams['pcyc_rad'] = qi*B0[ii] / mp                                       # Proton cyclotron frequency
        
            #savename = DR_save_path + 'DR_{:04}.png'.format(ii)
            savename = DR_save_path + file[32:-4] + '_{:04}.png'.format(ii)
    
            plot_dispersion(all_k[ii], all_CPDR[ii], all_WPDR[ii], PlasParams,
                            norm_k=False, norm_w=False, save=True, savename=savename,
                            title=times[ii], growth_only=False, glims=0.05)
    return
...
def plot_raw_DR_file():
    '''
    Designed to just output raw info. k values seem fine, its the w calculation thats messed.
    
    Maybe just do He spectrum for ease
    
    How to plot with time? newtime is 1000 points, oldtime is 550.
    -- Set time[ii] to be ii/times.shape[0] to get value between 0-1
    '''
    files = ['DR_results_coldcomp_070_020_010_20130725_2100_2200_oldtime_nofilt.npz',
             'DR_results_coldcomp_070_020_010_20130725_2100_2200_oldtime_filt.npz',
             'DR_results_coldcomp_070_020_010_20130725_2100_2200_newtime_nofilt.npz',
             'DR_results_coldcomp_070_020_010_20130725_2100_2200_newtime_filt.npz'] 
    
    fig, ax = plt.subplots()
    
    # Extract max growth for He+ only for each file, WPDR only
    for file in files:
        data_pointer = np.load(save_dir + file)
        all_WPDR     = data_pointer['all_WPDR']         # Shape (time, k, species)

        Nt    = all_WPDR.shape[0]
        time  = np.arange(Nt) / Nt
        max_g = np.zeros(Nt, dtype=float)
        for ii in range(Nt):
            max_g[ii] = all_WPDR[ii, :, 1].imag.max()
            
        ax.plot(time, max_g, label=file[-18:-4], marker='x')
...
if __name__ == '__main__':
    ...
    #get_all_DRs(time_start, time_end, probe, pad, cmp=[0.7, 0.2, 0.1], nsec=None, suff='_oldtime_nofilt',
    #            HM_filter_mhz=None)
    
# =============================================================================
#     times1, B01, name1, mass1, charge1, density1, tper1, ani1, cold_dens1 = \
#     extract_species_arrays(time_start, time_end, probe, pad, cmp=[0.7, 0.2, 0.1], 
#                            return_raw_ne=True, nsec=None, HM_filter_mhz=None)
#     
#     times2, B02, name2, mass2, charge2, density2, tper2, ani2, cold_dens2 = \
#     extract_species_arrays(time_start, time_end, probe, pad, cmp=[0.7, 0.2, 0.1], 
#                            return_raw_ne=True, nsec=1, HM_filter_mhz=None)
# =============================================================================
...
#     axes[0].scatter(times1, B01, marker='x', c='k')
#     axes[0].plot(times2, B02, c='b')    
#     
#     axes[1].scatter(times1, density1[sp1:sp2].T, marker='x', c='k')
#     axes[1].plot(times2, density2[sp1:sp2].T, c='b')
#     
#     axes[2].scatter(times1, tper1[sp1:sp2].T, marker='x', c='k')
#     axes[2].plot(times2, tper2[sp1:sp2].T, c='b')
#     
#     axes[3].scatter(times1, ani1[sp1:sp2].T, marker='x', c='k')
#     axes[3].plot(times2, ani2[sp1:sp2].T, c='b')
#     
#     axes[4].scatter(times1, cold_dens1, marker='x', c='k')
#     axes[4].plot(times2, cold_dens2, c='b')
# =============================================================================
    
    #load_and_plot_timeseries()
    
# =============================================================================
#     # 10 Runs put on hybrid code
#     dayy  = '2013-07-25T'
#     timee = ['21:30:04.105',
#             '21:30:50.105',
#             '21:32:21.605',

...
def get_dispersion_relation(field_, ndensc_, ndensw_, A_, t_perp_, ndensw2_, A2_, t_perp2_, k_isnormalized=False, w_isnormalized=False, Nk=1000, kmin=0.0, kmax=1.0, plot=False, save=False, savepath=None):
    '''
...
if __name__ == '__main__':
    ...
    for ii, _var in zip(range(variable.shape[0]), variable):
        ...
        _ndensw    = np.zeros(Nn)
        _ndensw[0] = 0.05*n0     # Warm Hydrogen density (/cc, warm species 1)
        _ndensw[1] = 0.0*n0      # Warm Helium
        _ndensw[2] = 0.0*n0      # Warm Oxygen
    
        _t_perp    = np.zeros(Nn)
        _t_perp[0] = 50000.     # Perpendicular temperature (ev)
        _t_perp[1] = 00000.
        _t_perp[2] = 00000.
    
        _A    = np.zeros(Nn)
        _A[0] = _var            # Ion species anisotropy : A = T_perp / T_par - 1
        _A[1] = 0.
        _A[2] = 0.
        
        _ndensw2    = np.zeros(Nn)
        _ndensw2[0] = 0.05*n0    # Warm Hydrogen density (/cc, warm species 2)
        _ndensw2[1] = 0.0*n0     # Warm Helium
        _ndensw2[2] = 0.0*n0     # Warm Oxygen
    
        _t_perp2    = np.zeros(Nn)
        _t_perp2[0] = 50000.    # Perpendicular temperature 2 (ev)
        _t_perp2[1] = 00000.
        _t_perp2[2] = 00000.
    
        _A2    = np.zeros(Nn)
        _A2[0] = _var           # Ion species anisotropy 2 : A = T_perp / T_par - 1
        _A2[1] = 0.
        _A2[2] = 0.
        ...
        if output == 'show':
            ...
            figsave_path = save_path + 'test_{}.png'.format(ii)

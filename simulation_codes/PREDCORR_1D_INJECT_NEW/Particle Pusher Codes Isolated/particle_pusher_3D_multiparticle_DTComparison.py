...
def call_and_plot():
    ...
    for dt_mult, tt in zip(dt_mults, np.arange(Nt)):
        ...
        # "Variance" of mu from mu0 over time period
        var_dt = ((mu*1e10 - init_mu*1e10) ** 2).sum(axis=0) / mu.shape[0]
        ...
        ax.set_title(r'Variance in $\mu$ ($\times 10^{10}$) vs DT :: Exact (3D) vs. Approx (1D) soln :: |v| = %4.1f$v_A$ :: $\alpha$ = %4.1f' % (v_mag, pitch))
        ax.set_ylabel(r'$\frac{\sum(\mu_i - \mu_0)^2}{N}$', rotation=0, fontsize=20, labelpad=40)
        ax.set_xlabel('$\Delta t$ multiplier')
# =============================================================================
#         if True:

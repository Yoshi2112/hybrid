...
def velocity_update(pos, vel, dt):
    ...
    for ii in nb.prange(vel.shape[1]):  
        ...
        #Bps[:, ii],rL[ii] = eval_B0_exact(pos[ii], v_minus, Bps[:, ii])     # Exact solution
...
def do_particle_run(max_rev=1000, DT_mult=1.0):
    ...
    Np         = 1
    v_mag      = 10 * va
    init_pitch = np.linspace(88, 84, Np) * np.pi / 180.
    vel        = np.zeros((3, Np))
...
def test_mirror_motion():
    ...
    for DT_multiplier in [1.0, 0.5, 0.25, 0.1, 0.01]:
        print('Doing DTx{}'.format(DT_multiplier))
        max_rev = 2500
        init_pos, init_vel, init_pitch, time, pos_history, rL_history, vel_history, mag_history, DT, max_t = do_particle_run(max_rev=max_rev, DT_mult=DT_multiplier)

...
from simulation_parameters_1D import dx, NX, ND, NC, N, kB, Nj, nsp_ppc,                  \
                                     ...
                                     r_damp, Bc, qm_ratios, freq_res, rc_hwidth
...
def uniform_gaussian_distribution_quiet():
    ...
    for jj in range(Nj):
        ...
        # Change how many cells are loaded between cold/warm populations
        if const.temp_type[jj] == 0:
            cell_min = 0
            cell_max = NX
        else:
            cell_min = NX // 2 - rc_hwidth
            cell_max = NX // 2 + rc_hwidth
        
        # Load particles in selected cell range
        for ii in range(cell_min, cell_max):
            st     = idx_start[jj] + acc

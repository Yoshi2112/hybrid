...
from simulation_parameters_1D import dx, NX, ND, NC, N, kB, Nj, dist_type, nsp_ppc,               \
                                     idx_start, idx_end, seed, Tpar, Tper, mass, drift_v, theta,  \
...
def particles_per_cell():
    ...
    for ii in range(Nj):
        ...
        # DELETE IF EXTRAPARTICLE THING DOESN"T WORK. 
        # ALSO DON"T FORGET THE +2*Nj IN SIMULATION_PARAMETERS
        ppc[ii, NX - 1] += 2
    return ppc
...
# =============================================================================
# @nb.njit()
# def uniform_gaussian_distribution_quiet_cellwise(ppc):
...
import pdb
#@nb.njit()
def uniform_gaussian_distribution_quiet_extraparticle(ppc):
    '''Creates an N-sampled normal distribution across all particle species within each simulation cell
    ...
    for jj in range(Nj):                    # For each species
        ...
        for ii in range(NX):                # For each cell
            ...
            for kk in range(half_n):        # For each particle in that cell (half only)
                ...
                if ii < NX - 1:
                    pos[st + kk] = dx*(float(kk) / half_n       + ii - NX // 2)
                else:
                    pos[st + kk] = dx*(float(kk) / (half_n - 1) + ii - NX // 2)
...
# =============================================================================
# @nb.njit()
# def uniform_gaussian_distribution_quiet_simspace(ppc):
#     '''Creates an N-sampled normal distribution across all particle species across the
#     simulation space.
# 
#     INPUT:
#         ppc -- Number of particles per cell to initialize
...
#         dist -- Output distribution: Maxwellian, as generated by numpy's random.normal in 3 dimensions
#         
#     THERE'S NOT REALLY ANY BENEFIT TO THIS 
#     '''
#     pos = np.zeros(N)
#     vel = np.zeros((3, N))      # Initialize array
#     idx = np.zeros(N, dtype=np.uint8)
...
#         
#         st     = idx_bounds[jj, 0]
#         en     = idx_bounds[jj, 0] + half_n
#             
#         sf_par = np.sqrt((kB *  Tpar[jj]) /  mass[jj])
#         sf_per = np.sqrt((kB *  Tper[jj]) /  mass[jj])
#             
#         for kk in range(half_n):        # For each particle (half only)
#             pos[st + kk] = xmax * float(kk) / (half_n - 1)
#                 
#         vel[0, st: en] = np.random.normal(0, sf_par, half_n) +  drift_v[jj]
#         vel[1, st: en] = np.random.normal(0, sf_per, half_n)
#         vel[2, st: en] = np.random.normal(0, sf_per, half_n)
...
def initialize_particles():
    ...
    pos, vel, idx = uniform_gaussian_distribution_quiet_extraparticle(ppc)

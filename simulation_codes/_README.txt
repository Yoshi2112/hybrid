...
PREDCORR_1D_PULSE
Basically the KLIMAS code but modified to include some sort of analytic wave generation at the equator, for testing purposes. Just a junk code to see if it even works. Need the generation in order to properly test and define the ABCs (with a uniform cold plasma). Will later be integrated if it works.
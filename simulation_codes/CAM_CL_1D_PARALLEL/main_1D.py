...
@nb.njit()
def do_subcycle(B1, B2, B_center, B_damp, Ji, J_ext, rho, E, Ve, Te, resistive_array, curl,
                dh, sim_time, half_sc, half_push):
    ...
    # Sync B2 with B1 if 2nd half-loop
    if half_push == 0:
        calculate_E(B1, B_center, Ji, J_ext, rho, E, Ve, Te, resistive_array, sim_time)
        get_curl_E_4thOrder(E, curl) 
        B2  -= dh * curl
...
@nb.njit()
def averageFields(B1, B2, num_av):
    '''
    Calculates the average of B1 and B2 and makes them identical
    '''
    B1 += B2; B1 *= 0.5
    B2[:] = B1[:]
    num_av += 1
...
def new_cyclic_leapfrog(B1, B2, B_center, rho, Ji, J_ext, E, Ve, Te, dt, subcycles,
                    ...
                    B_thresh_error=1e-4, check_error=0, half_push=0, num_av=0):
    '''
    Check:
    ...
    TODO:
        -- Code to allow either the first push (half push needed, no sync) or the
            second (no half push, sync required).
        -- When criteria met (either every 5dt, or err>threshold), average B1, B2
            at end of function (since the next loop will do a half push anyway)
        
    Questions:
        -- Are two half-pushes equivalent to an unbroken subcycle? I.e. is doing
            it this way going to affect accuracy? Probably not to any substantial degree,
            but should still check.
        -- When to increase subcycle count? If its done any time the solutions diverge,
            we could end up with an infinite number of subcycles. Also, needs to be
            a flag at some point to determine when the timestep should be halved instead.
            ANSWER: Average and increase s/c count each time, but lower the max number of
                subcycles down to something like 24-32. Once beyond this, halve the timestep
                instead.
        
    NOTE: The hybrid implementation of this algorithm seems to preclude redoing cycles
        if convergence isn't found, since half the cycles depend on J, rho at N=0 and
        the other half at J, rho=1, it's not as simple as just calling the function
        again with a greater number of subcycles.
        Best that seems to be possible is to average and flag for more subcycles in the
        next timestep.
        
    1st half-loop: Half-push
    2nd half-loop: Calc B* = B2
    Average if err>thresh or subcycle/timestep changes
    Subcycle changed if dispersion unresolved or too many averages (put in a counter for this)
    Timestep halved if max_subcycle exceeded
    
    TODO treat each half-push as its own thing under cyclic leapfrog. If the desired accuracy
        isn't attained, run again with more subcycles. If these don't converge, output the 
        fields to find out why (i.e. treat the maths! Look at the Modified-Midpoint Method)
    '''
    half_sc = subcycles//2
    H     = 0.5 * dt
    dh    = H / half_sc
    ...
    sim_time = do_subcycle(B1, B2, B_center, B_damp, Ji, J_ext, rho, E, Ve, Te, resistive_array, curl,
                    dh, sim_time, half_sc, half_push)
    
    if check_error:
        B_error = np.abs(B2 - B1).max()
...
#@nb.njit()
def check_timestep(qq, DT, pos, vel, idx, Ie, W_elec, Ib, W_mag, mp_flux, B1, B2, B_center, E, dns, 
                   max_inc, part_save_iter, field_save_iter, loop_save_iter,
                   subcycles, B_damping_array, resistive_array, num_av):
    '''
    ...
    if True:
        ...
        if subcycles < 0.75*new_subcycles or num_av >= 5:                                       
            subcycles *= 2; ch_sc = 1
            print('Number of subcycles per timestep doubled to', subcycles)
            
        if False and subcycles > 3.0*new_subcycles and subcycles%4 == 0:                                      
            subcycles //= 2; ch_sc = 1
...
if __name__ == '__main__':
    ...
    _QQ = 1; _SIM_TIME = 0.0; _CHECK_ERR=0; _NUM_AV=0; start_time = timer()
    print('Starting loop...')
    while _QQ < _MAX_INC:
        loop_start = timer()
        if print_timings:

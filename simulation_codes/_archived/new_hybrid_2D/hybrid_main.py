...
if __name__ == '__main__':                         # Main program start
    ...
    for qq in range(maxtime):
        if qq == 0:
            ...
            part = particles.velocity_update(part, B[:, :, 0:3], E[:, :, 0:3], -0.5*DT, W, Wb)  	  # Retard velocity to N - 1/2 to prevent numerical instability
            
            #cc.check_cell_dist_2d(part, [10,10], 0)
        else:
            # N + 1/2
            # pdb.set_trace()
            part          = particles.velocity_update(part, B[:, :, 0:3], E[:, :, 0:3], DT, W, Wb)  # Advance Velocity to N + 1/2
            part, W, Wb   = particles.position_update(part, DT)                                     # Advance Position to N + 1
            B[:, :, 0:3]  = fields.push_B(B[:, :, 0:3], E[:, :, 0:3], DT)                           # Advance Magnetic Field to N + 1/2
            
            dns           = 0.5 * (dns + sources.collect_density(part, W))                          # Collect ion density at N + 1/2 : Collect N + 1 and average with N                                             
            ...
            part         = particles.velocity_update(part, B[:, :, 0:3], E[:, :, 0:3], DT, W, Wb)   # Advance particle velocities to N + 3/2
            part, W, Wb  = particles.position_update(part, DT)                                      # Push particles to positions at N + 2
            dns          = 0.5 * (dns + sources.collect_density(part, W))                           # Collect ion density as average of N + 1, N + 2

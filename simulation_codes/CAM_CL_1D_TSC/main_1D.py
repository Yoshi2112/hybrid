...
if __name__ == '__main__':
    ...
    while qq < max_inc:
        ...
        ##### EXAMINE TIMESTEP #####
        ############################
        pos, qq, DT, maxtime, data_iter, plot_iter, change_flag, subcycles = aux.check_timestep(qq, DT, pos, vel, B, E, dns_int, max_inc, data_iter, plot_iter, subcycles)
        ...
        E, Ve = fields.calculate_E(B, J_minus, dns_half)
        J     = sources.push_current(J_plus, E, B, L, G, DT)
        E, Ve = fields.calculate_E(B, J, dns_half)
        ...
        E, Ve   = fields.calculate_E(B, J, dns_int)                                     # This one's just for output

        ####################################
        ##### OUTPUT DATA AND/OR PLOTS #####
        ####################################
        if qq%data_iter == 0 and generate_data == 1:                                   # Save data, if flagged
            pas.save_data(DT, data_iter, qq, pos, vel, J, E, B, Ve, dns_int)

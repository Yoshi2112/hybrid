...
from const             import NX, NY, dx, dy, Te0, mhd_equil, q, mu0, kB
...
def push_B(B, E, dt):   # Basically Faraday's Law. (B, E) vectors
    ...
    for mm in range (1, NX + 1):
        for nn in range(1, NY + 1):
            B[mm, nn, 0] = B[mm, nn, 0] - (dt / 2.) * (  (E[mm, nn + 1, 2] - E[mm, nn - 1, 2]) / (2 * dy))
            B[mm, nn, 1] = B[mm, nn, 1] + (dt / 2.) * (  (E[mm + 1, nn, 2] - E[mm - 1, nn, 2]) / (2 * dx))
            B[mm, nn, 2] = B[mm, nn, 2] - (dt / 2.) * ( ((E[mm + 1, nn, 1] - E[mm - 1, nn, 1]) / (2 * dx)) - ((E[mm, nn + 1, 0] - E[mm, nn - 1, 0]) / (2 * dy)) )
...
def push_E(B, J_i, n_i, dt):
    ...
    E_out = np.zeros((NX + 2, NY + 2, 3))                 # Output array - new electric field
    JxB   = np.zeros((NX + 2, NY + 2, 3))                 # V cross B holder
    BdB   = np.zeros((NX + 2, NY + 2, 3))                 # B cross del cross B holder     
    del_p = np.zeros((NX + 2, NY + 2, 3))                 # Electron pressure tensor gradient array
    J     = np.zeros((NX + 2, NY + 2, 3))                 # Ion current
    qn    = np.zeros((NX + 2, NY + 2   ))                 # Ion charge density

    Te = np.ones((size, size)) * Te0                      # Electron temperature array
    ...
    for xx in range(3):
        ...
        BdB[  :, :, xx] /= qn[:, :]*mu0
        del_p[:, :, xx] /= qn[:, :]
        E_out[:, :, xx]  = - JxB[:, :, xx] - del_p[:, :, xx] - BdB[:, :, xx]
...
def push_E_equil(B, J_i, n_i, dt):
    ...
    BdB   = np.zeros((NX + 2, NY + 2, 3))                                 # B cross del cross B holder     
    del_p = np.zeros((NX + 2, NY + 2, 3))                                 # Electron pressure tensor gradient array
    J     = np.zeros((NX + 2, NY + 2, 3))                                 # Total ion current
    qn    = np.zeros((NX + 2, NY + 2   ))                                 # Ion charge density
    ...
    Te  = np.ones(size, size) * Te0           # Electron temperature per cell

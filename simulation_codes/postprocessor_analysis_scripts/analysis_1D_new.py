...
def manage_dirs(create_new=True):
    global run_dir, data_dir, anal_dir, temp_dir, base_dir, field_dir, particle_dir
...
def load_fields(ii, fields=True, particles=True):    
...
def plot_wx(component='By', linear_overlay=False):
    ...
    x  = np.arange(NX)
    f  = np.fft.fftfreq(time_seconds_field.shape[0], d=dt_field)
...
def plot_wk(component='By', dispersion_overlay=False, plot=False, save=False):
    ...
    df = 1. / (num_times * dt_field)
    dk = 1. / (NX * dx)

    f  = np.arange(0, 1. / (2*dt_field), df)
...
def get_derivative(arr):
    ...
    deriv[0 ] = (-3*arr[ 0] + 4*arr[ 1] - arr[ 2]) / (2 * dt_field)
    deriv[-1] = ( 3*arr[-1] - 4*arr[-2] + arr[-3]) / (2 * dt_field)
    
    for ii in np.arange(1, arr.shape[0] - 1):
        deriv[ii] = (arr[ii + 1] - arr[ii - 1]) / (2 * dt_field)
    return deriv
...
def get_max_frequency(arr, plot=False):
    ...
    '''
    npts      = arr.shape[0]
    fft_freqs = np.fft.fftfreq(npts, d=dt_field)
...
def fit_fied_component(arr, fi, component, cut_idx=None, plot=False, plot_cell=64):
    ...
    time_fit  = time_seconds_field[:cut_idx]
...
def fit_magnetic_energy(by, bz, plot=False):
    ...
    time_fit = time_seconds_field[:linear_cutoff]
    ...
    if plot != None:
        plt.ioff()
        plt.figure()
        plt.plot(time_seconds_field[:linear_cutoff], U_B[:linear_cutoff], color='green', marker='o', label='Energy')
        plt.plot(time_seconds_field[:linear_cutoff], fit_function, color='b', label='Exp. fit')
